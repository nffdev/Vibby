...
export function AuthWrapper({ children }) {
	...
	useEffect(() => {
		if (!['/profile', '/upload', '/videoscreen', '/settings'].some(path => window.location.pathname.startsWith(path)) || !auth || (user && user.id)) return;
		setIsLoading(true);
	...
	if (!['/profile', '/upload', '/videoscreen', '/settings'].some(path => window.location.pathname.startsWith(path))) return <>{children}</>;
